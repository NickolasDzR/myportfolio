...
var templates = {
		Stream: '<div class="gha-feed">{{{text}}}',
		...
		UserHeader: '<div class="gha-header">\
				   <div class="gha-github-icon"><span class="octicon octicons-custom"><svg><use xlink:href="#octicons-mark-github"></use></svg></span></div>\
				   <div class="gha-user-info{{withoutName}}"><p>{{{userLink}}}</p></div>\
...
GitHubActivity.feed({
	username: "NickolasDzR",
	selector: "#gha_widget",
	limit: 5
});

/* progressbar layout starts */
const fillProgressBar = (observedElement) => {
	const progresBarElements = observedElement.querySelectorAll('.progress-bar');
	for (let i = 0; i < progresBarElements.length; i += 1) {
		const widthProgressBar = progresBarElements[i].children[0].getAttribute("data-level");
		setTimeout(fillProgressBarSetInterval, 1000, widthProgressBar, progresBarElements[i].children[0]);
	}
}

const fillProgressBarSetInterval = (widthProgressBar, pb) => {
	setInterval(style, 13);
	let width = 0;

	function style() {
		if (width > widthProgressBar) {
			clearInterval(style);
		} else {
...
const progressBards = document.querySelector(".progress-bar");

// check the width of progress-bar element, if first element has width, then don't change the width of all elements
function checkWidthProgressBar(progresBarElements) {
	return progresBarElements[0].children[0].clientWidth;
}

const observer = new IntersectionObserver((entries) => {
	const progressBarElements = entries[0].target.querySelectorAll('.progress-bar');
	if (checkWidthProgressBar(progressBarElements) > 0) {
		return;
	}
	if (entries[0].isIntersecting === true) {
		if (progressBards.getAttribute("style") === null) {
			fillProgressBar(entries[0].target);
		}
		return;
	}
});

const mainContentItem1 = document.querySelector('.main-content__observer_item-1');
const mainContentItem2 = document.querySelector('.main-content__observer_item-2');
observer.observe(mainContentItem1);
observer.observe(mainContentItem2)
/* progressbar layout ends */

const animationCloseModal = (parentWindow, childWindow) => {
	childWindow.classList.add('close-modal-top');
	setTimeout(() => {
		parentWindow.classList.add('close-modal-left');
	}, 300);
	setTimeout(function () {
		parentWindow.classList.add('close-modal-left');
	}, 700)
...
const openSwalModal = (parentWindow, childWindow) => {
	childWindow.classList.add('close-modal-top');
	setTimeout(() => {
		Swal.fire({
			icon: 'success',
			title: "Спасибо!",
			text: 'Я скоро с вами свяжусь',
			showClass: {
				popup: 'animated slideInUp'
			},
			hideClass: {
				popup: 'animated close-modal-top'
			},
...
const portfolioProjectBlock = document.querySelectorAll('.main-content__content-b')[1];

portfolioProjectBlock.addEventListener('click', e => {
	e.preventDefault();
	if (document.getElementById('lightbox') === null) {
		if (e.target.nodeName === 'IMG' && e.target.classList.contains('img-profile') !== true && e.target.id !== "sertificate" && e.target.id !== 'lightboxImg') {
			const lightbox = document.createElement('div');
			lightbox.id = 'lightbox';
			lightbox.innerHTML = '<div id="lightboxInner"><a class="closeButton"></a><img id="lightboxImg" class="img-position"><div class="project-link-wrp"><a id="project-link">Посмотреть проект</a></div></div>';
...
document.addEventListener('click', (e) => {
	...
	if (e.target.className === 'closeButton' || event.target.id === 'lightbox' || event.target.id === 'lightboxInner') {
		animationCloseModal(lightBox, lightBoxInner);
	}
})
...
document.addEventListener('input', (e) => {
	const deleteErrorStyle = (elem) => {
		const input = document.getElementById(elem);
		input.parentElement.classList.remove('input-error-style');
	}
	for (let i = 0; i < e.target.parentElement.classList.length; i += 1) {
		if (e.target.parentElement.classList[i] === 'input-error-style') {
			deleteErrorStyle(e.target.id)
		}
	}
})
...
document.addEventListener('click', (e) => {
	...
	if (e.target.id === "modal__close" || e.target.id === "modal" || e.target.id === "blur-modal") {
